...
import { CurvedArrow, Puzzle, Stop, Paperclip, AtSymbol } from '../icons/icons';
  ...
  return (
    <div className={cn('flex h-full flex-col', isFullScreen ? 'mx-auto max-w-xl' : '')}>
                    ...
                    <div
                      className={cn(
                        ...
                        message.role === 'user'
                          ...
                          : 'overflow-wrap-anywhere mr-auto break-words px-2',
                      )}
                    >
                      {textParts.map((part) =>
                        part.text && (
      ...
      <div className={cn('mb-4 flex-shrink-0 px-4', isFullScreen ? 'px-0' : '')}>
              ...
              <div className="w-full">
                <form id="ai-chat-form" onSubmit={handleSubmit} className="relative">
                  <AITextarea
                    ...
                    className="bg-subtleWhite rounded-lg dark:bg-[#141414]"
                    onCommandEnter={() => {
                      if (input.trim() && chatMessages.enabled) {
                  ...
                  {status === 'ready' ? (
                    <button
                      form="ai-chat-form"
                      type="submit"
                      className="absolute right-1.5 top-1/2 inline-flex h-6 -translate-y-1/2 cursor-pointer items-center justify-center gap-1.5 overflow-hidden rounded-lg"
        ...
        <button className="flex h-6 w-fit cursor-pointer items-center justify-between gap-1 rounded-md border-0 px-2 text-xs hover:bg-[#1E1E1E] dark:bg-[#141414]">
            <div className="flex items-center gap-1.5">
              <Paperclip className="h-3.5 w-3.5 text-white dark:text-[#929292]" />
            </div>
          </button>
          <button className="flex h-6 w-fit cursor-pointer items-center justify-between gap-1 rounded-md border-0 px-2 text-xs hover:bg-[#1E1E1E] dark:bg-[#141414]">
            <div className="flex items-center gap-1.5">
              <AtSymbol className="h-3 w-3 fill-white dark:fill-[#929292]" />
            </div>
          </button>
        </div>
        <div className="flex items-center justify-end gap-1">
          <div className="relative z-10 mt-1 flex items-center justify-end">
            <Select>
              <SelectTrigger className="flex h-6 w-fit cursor-pointer items-center justify-between gap-1 border-0 px-2 text-xs hover:bg-[#1E1E1E] dark:bg-[#141414]">
                <div className="flex w-full items-center gap-1.5">
                  <Puzzle className="h-3.5 w-3.5 fill-white dark:fill-[#929292]" />
                </div>
              </SelectTrigger>
              <SelectContent className="w-36 rounded-md border-0 bg-[#1E1E1E] p-0.5 shadow-md">
                <SelectItem
                  value="gpt-3.5"
